...
class BroadWorks:
    ...
    def __init__(self, url, username, password, location):
        self._url = url
        self._username = str(username)
        self._password = str(password)
        ocilocation = str(location)

        self._sessionId = uuid.uuid4()
        self._conn = Client(self._url, faults=True, location=ocilocation)
    ...
    def _sendRequest(self, schema):
        ...
        r['type'] = command.get('{http://www.w3.org/2001/XMLSchema-instance}type')
    ...
    def LoginRequest22V4(self):
            ...
            if auth:
                ...
                signedPasswordHash.update(bytes("{}:{}".format(auth['data']['nonce'], secret), 'utf-8'))
                signedPassword = signedPasswordHash.hexdigest()
                resp = self._sendRequest(OCISchema.LoginRequest22V4(self._username, signedPassword))
...
class OCISchema:
    ...
    @staticmethod
    def LoginRequest22V4(userId, signedPassword):
        name = 'LoginRequest22V4'
        elements = OrderedDict()
        elements['userId'] = userId
        elements['signedPassword'] = signedPassword
        tables = []
